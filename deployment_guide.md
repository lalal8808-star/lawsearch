...
백엔드에는 아래 선택 환경변수를 설정할 수 있습니다.

- `MONTHLY_AI_TOKEN_LIMIT`: 사용자별 월간 토큰 한도. 기본값 `2000000`.
- `AI_INPUT_USD_PER_MILLION`: 사용 모델의 입력 100만 토큰당 USD. 미설정 시 비용은 `0`으로 두고 토큰만 집계.
- `AI_OUTPUT_USD_PER_MILLION`: 사용 모델의 출력 100만 토큰당 USD. 미설정 시 비용은 `0`으로 두고 토큰만 집계.

모델 가격이 변경될 수 있으므로 가격 값은 코드에 고정하지 않고 배포 환경변수로 관리합니다.
